...
const slice = createAppSlice({
	...
	reducers: (creators) => {
		...
		return {
			...
			// 	// 1 var
			// 	const todoLists = state.find((e) => e.id === action.payload.todoListId);
			// 	if (todoLists) {
			// 		todoLists.title = action.payload.title;
			// 	}
			// 	// 2 var
	...
	//     const todoLists = state.find((e) => e.id === action.payload.todoListId);
	//     if (todoLists) {
	//       todoLists.filter = action.payload.filter;
	//     }
	//     // const index = state.findIndex((e) => e.id === action.payload.todoListId);
	//     // if (index > -1) {
	//     //   state[index].filter = action.payload.filter;
	...
	//     const todoLists = state.find((e) => e.id === action.payload.todoListId);
	//     if (todoLists) {
	//       todoLists.entityStatus = action.payload.status;
	//     }
	//     // return state.map((t) => (t.id === action.payload.todoListId ? { ...t, entityStatus: action.payload.status } : t));
	//   },
